    ...
    function averageFinalists() {
        return new Promise( async ( resolve, reject ) => {
            ...
            try {
                await client.connect();
                const db = client.db( dbName );
                const average = await db.collection( 'newspapers' ).aggregate( [ { $group: { _id: null, avgFinalists: { $avg: '$Pulitzer Prize Winners and Finalists, 1990-2014' } } } ] ).toArray()
        ...
        return new Promise( async ( resolve, reject ) => {
            ...
            try {
                ...
                const average = await db.collection( 'newspapers' ).aggregate( [
                    { $project: { 'Newspaper': 1, 'Pulitzer Prize Winners and Finalists, 1990-2014': 1, 'Change in Daily Circulation, 2004-2013': 1, overallChange: { $cond: { if  :  { $gte: [ '$Change in Daily Circulation, 2004-2013', 0 ] }, then: 'positive', else  : 'negative'  } } } }, { $group: { _id: '$overallChange', avgFinalists: { $avg: '$Pulitzer Prize Winners and Finalists, 1990-2014' } } }
